...
export class FormHeroesComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.activatedRoute.params.subscribe(
      ...
      (err: string) => console.error('Error Occured When search hero for id ' + err),
  ...
  save() {
    ...
    if (this.hero.id) {
      ...
      this.heroesService.addHero(this.heroForm.value)
        ...
        (err: string) => console.error('Error Occured When Add A New Heroe ' + err),
        () => (this.showLoading = false);
    };
  };
  ...
  loadForm(hero: Hero): void {
    this.heroForm = this.formBuilder.group({
      ...
      originators: [hero.originators ? hero.originators : [], Validators.required],
  ...
  add(event: MatChipInputEvent, attribute: string): void {
    ...
    if (value) {
      this.heroForm.controls[attribute].setValue([...this.heroForm.controls[attribute].value, value.trim()]);
